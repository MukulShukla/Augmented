...
//writes at center of image, returns the coordinate where the write was made
...
//writes on image at fin coordinates
...
int main( int argc, char* argv[] ) {
	...
	int i=0,N=32,e,j,q;
	//Correct usage guide; program terminates if arguments are not provided
	if(argv[1]==NULL || argv[2]==NULL){
		cout<<"Invalid arguments\nUse as:\n";
		cout<<"addgesture [geture-character] [custom-command]\n\n";
		return -1;
		}
	// gesture command is extracted
	cmmd = argv[1];
	g = cmmd[0]; i=0;
	
	//Checking if gesture exists and cannot be overwritten
	while(str1[i]!='\0'){
		if (g == str1[i]){
	...
	//Capture a frame, and extract gesture name
	cmmd = argv[2];
	...
	for (e=0 ; e<1 ; e++){
		...
		while (true) {	
				...
				//get frames and break if frame is empty
				if (!frames) break;
				//get frame difference
				differ(mid,frames);
				crosshair(mid);
				
				//start recording on space, exit on Esc, break if gesture is too long
  ...
  //cout<<"Gesture succesfully saved.\n";
	...
	//open files, if gesture is to be overwritten
